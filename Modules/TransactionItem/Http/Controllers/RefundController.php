...
use Modules\TransactionItem\Entities\ProductItem;
...
use Modules\TransactionItem\Http\Requests\RefundRequest;
...
class RefundController extends Controller
{
    ...
    public function index(FilterRequest $request)
    {
        ...
        return view('transactionitem::content/refund/view',compact([
    ...
    public function store(RefundRequest $request)
    {
        ...
        if (Hash::check($request->pin, $storeManager->password)) {
            ...
            try {
                ...
                /* Inventory Process */
                $product = ProductItem::where(['code' => $itemInfo->item->code])->first();
                $stock = ProductStock::where(['item_id' => $product->id])->first();
                if ($request->quantity <= $itemInfo->quantity) {
                    ...
                    flash('Refund Berhasil, Item '.$product->name.' dengan jumlah item yang di kembalikan x'.$request->quantity)->success();
                    return redirect()->route('transItemDetail',['number'=> $request->transaction_number]);
                }else{
